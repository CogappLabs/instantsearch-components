...
/** The search state the view draws, from whichever InstantSearch flavour. */
export interface DateHistogramState {
  /** The facet's bounds, from its stats. */
  range: { min?: number; max?: number };
  /** The refinement: an open end is undefined or infinite. */
  start: readonly [number | undefined, number | undefined];
  refine: (range: [number | undefined, number | undefined]) => void;
  /** Per-value counts, as a refinement list reports them. */
  items: readonly { value: string; count: number }[];
}

/**
 * A year range picked with two handles under a histogram of the records'
 * years, or typed. The view alone: `DateHistogram` feeds it from React
 * InstantSearch, `dateHistogram` from InstantSearch.js.
 *
 ...
 * the year boxes take any year inside the data's range.
 */
export const DateHistogramView = ({
  range,
  start,
  refine,
  items,
  ...
  showCount = true,
  className,
}: Omit<DateHistogramProps, "attribute" | "valueLimit"> & DateHistogramState) => {
